...
export function FiltrosEntradas({
  busca, onBuscaChange, de, onDeChange, ate, onAteChange, status, onStatusChange,
}: Props) {
  ...
  const temFiltroAtivo = !!busca || !!de || !!ate || !!status
  const statusLabel = STATUS_OPTIONS.find(s => s.valor === status)?.rotulo ?? 'Status'
  ...
  const limparTudo = () => { onBuscaChange(''); onDeChange(''); onAteChange(''); onStatusChange('') }
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 20px', borderBottom: '1px solid var(--ada-border-sub)', position: 'relative' }}>
        <div onFocus={() => setFocado(true)} onBlur={() => setFocado(false)} style={{
          ...
          <label htmlFor="busca-entradas" className="sr-only">Buscar entrada</label>
          <input id="busca-entradas" type="text" placeholder="Buscar por fornecedor ou nota fiscal…" value={busca} onChange={e => onBuscaChange(e.target.value)} autoComplete="off" style={{ flex: 1, background: 'none', border: 'none', outline: 'none', fontSize: 14.5, fontWeight: 500, color: 'var(--ada-heading)', letterSpacing: '-.005em', height: '100%' }} />
          {busca && (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 20px', borderBottom: '1px solid var(--ada-border-sub)', overflowX: 'auto', scrollbarWidth: 'none' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
          <label htmlFor="entradas-de" style={{ fontFamily: 'ui-monospace, monospace', fontSize: 10.5, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--ada-placeholder)', flexShrink: 0 }}>De</label>
          <input id="entradas-de" type="date" value={de} max={ate || undefined} onChange={e => { const v = e.target.value; onDeChange(v); if (ate && v.length === 10 && v > ate) onAteChange('') }} style={dateInputStyle(!!de)} />
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
          <label htmlFor="entradas-ate" style={{ fontFamily: 'ui-monospace, monospace', fontSize: 10.5, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--ada-placeholder)', flexShrink: 0 }}>Até</label>
          <input id="entradas-ate" type="date" value={ate} min={de || undefined} onChange={e => { const v = e.target.value; onAteChange(v); if (de && v.length === 10 && v < de) onDeChange('') }} style={dateInputStyle(!!ate)} />
        </div>
        ...
        <div style={{ position: 'relative' }}>
          <button ref={btnRef} type="button" onClick={abrirDropdown} aria-expanded={dropdownAberto} aria-haspopup="listbox" style={{
            ...
            background: status ? 'linear-gradient(180deg, rgba(240,176,48,.10), rgba(212,150,12,.06))' : 'var(--ada-surface-2)',
            border: `1px solid ${status ? 'rgba(240,176,48,.35)' : 'var(--ada-border)'}`, borderRadius: 9,
            fontSize: 12.5, fontWeight: 500, color: status ? 'var(--ada-heading)' : 'var(--ada-body)',
            cursor: 'pointer', userSelect: 'none', whiteSpace: 'nowrap', flexShrink: 0, transition: 'all 150ms ease',
            boxShadow: status ? '0 0 0 1px rgba(240,176,48,.10) inset, 0 4px 12px -4px rgba(240,176,48,.35)' : 'none',
          }}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, color: status ? '#F0B030' : 'var(--ada-muted)', transition: 'color 150ms' }} aria-hidden="true">
          ...
          {dropdownAberto && createPortal(
            <div data-filtros-dropdown style={{
              ...
              minWidth: 180, padding: 6, animation: 'pillIn 180ms cubic-bezier(.34,1.56,.64,1)',
            }}>
              {STATUS_OPTIONS.map(opt => (
                <button type="button" key={opt.valor} onClick={() => { onStatusChange(opt.valor); setDropdownAberto(false) }} style={{
                  display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '7px 10px',
                  borderRadius: 7, fontSize: 13, color: status === opt.valor ? 'var(--ada-heading)' : 'var(--ada-body)',
                  cursor: 'pointer', border: 'none', background: status === opt.valor ? 'var(--ada-surface-2)' : 'none',
                  transition: 'background 100ms', fontFamily: 'inherit', textAlign: 'left',
                }}>
                  <span style={{ width: 16, height: 16, borderRadius: 4, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', border: status === opt.valor ? 'none' : '1.5px solid var(--ada-border)', background: status === opt.valor ? '#D4960C' : 'none' }}>
                    {status === opt.valor && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#0A0E16" strokeWidth="3" strokeLinecap="round"><path d="m4 12 5 5L20 6" /></svg>}
                  </span>
                  {opt.rotulo}
                </button>
              ))}
      ...
      {temFiltroAtivo && (
        <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 8, padding: '12px 20px', background: 'linear-gradient(90deg, rgba(212,150,12,.04) 0%, transparent 60%)', borderBottom: '1px solid var(--ada-border-sub)' }}>
          ...
          {status && (
            <span style={pillBase}>
              <span style={pillTag}>Status</span>
              <span style={{ padding: '5px 9px' }}>{statusLabel}</span>
              <button type="button" onClick={() => onStatusChange('')} aria-label="Remover filtro de status" style={pillClose}><XIcon /></button>
            </span>
          )}
