...
export function FiltrosInventarios({ busca, onBuscaChange, status, onStatusChange }: Props) {
  const [focado, setFocado] = useState(false)
  const [dropdownAberto, setDropdownAberto] = useState(false)
  const [dropdownPos, setDropdownPos] = useState({ top: 0, left: 0 })
  const btnRef = useRef<HTMLButtonElement>(null)

  const temFiltroAtivo = !!busca || !!status
  ...
  const limparTudo = () => { onBuscaChange(''); onStatusChange('') }
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 20px', borderBottom: '1px solid var(--ada-border-sub)', position: 'relative' }}>
        <div onFocus={() => setFocado(true)} onBlur={() => setFocado(false)} style={{
          ...
          <label htmlFor="busca-inventario" className="sr-only">Buscar inventário</label>
          <input id="busca-inventario" type="text" placeholder="Buscar por descrição…" value={busca} onChange={e => onBuscaChange(e.target.value)} autoComplete="off" style={{ flex: 1, background: 'none', border: 'none', outline: 'none', fontSize: 14.5, fontWeight: 500, color: 'var(--ada-heading)', letterSpacing: '-.005em', height: '100%' }} />
          {busca && (
